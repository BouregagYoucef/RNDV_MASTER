...
"""
stf = setup test files
rtf = run test files
rtf 1 --- test database connection and initialization
rtf 2 --- test default settings and theme
rtf 3 --- test add/retrieve user for login
rtf 4 --- test license manager integration
rtf 5 --- test add client
"""
...
def run_database_tests():
    ...
    try:
        ...
        print("❌ rtf 2 -- failed to retrieve settings/theme:", e)
        return

    if settings and theme:
        print("✅ rtf 2 --default settings of company has imported successfully",settings.get('company_name'))
        print("✅ rtf 2 --default theme has imported successfully",theme.get('Primary'))
    else:
        print("❌ rtf 2 --failed import theme/settings because they are None")
        return
    ...
    if user_id:
        print("✅ rtf 3 -- the user has added successfully:", user_id)
        user = db_manager.get_user_by_username('test_reception')
        if user and user.get('password_hash') == test_password_hash:
            print("✅ rtf 3 -- the user has retrieved successfully for login.")
        else:
            print("❌ rtf 3 -- failed to retrieve user after addition.")
    else:
        print("❌ rtf 3 -- failed to add user.")
    ...
    try:
        ...
        print("✅ rtf 4 -- license info has added to DB for testing.")
    except Exception as e:
        print(f"❌ rtf 4 -- failed to add license info to DB: {e}")

    print("\n--- 🔑 testing LicenseManager ---")
    try:
        license_manager = LicenseManager(db_manager)
        license_manager.check_activation_status()
        current_id = license_manager.get_current_machine_id()
        license_info = db_manager.get_license_info()
        print("🔑 rtf 4 License Info from DB:", license_info)
        print("🔑 rtf 4 Current Machine ID:", current_id)
        if license_info and current_id:
            print(f"✅ rtf 4 -- machine id has generated: {current_id[:10]}...")
            print(f"✅ rtf 4 -- status: {'active' if license_info.get('is_active') == 1 else 'inactive'}")
        else:
            print("❌ rtf 4 -- failed to generate or store license/machine information.")

    except Exception as e:
        print(f"❌ rtf 4 -- failed to initialize/run LicenseManager: {e}")
    ...
    if client_id:
        print(f"✅ rtf 5 -- the client has added successfully. ID = {client_id}")
    else:
        print("❌ rtf 5 -- failed to add client.")
