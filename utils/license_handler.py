    ...
    def _load_public_key(self) -> Optional[Any]:
        ...
        try:
            with open(PUBLIC_KEY_PATH, "rb") as key_file:
                ...
                print("🔑 Key File Content Preview:", key_file.read())  # عرض أول 50 بايت
            ...
            print("🔑 Failed to load public key.",PUBLIC_KEY_PATH)
            return None
...
# --- الدوال الرئيسية للتفعيل والتحقق ---
    ...
    def activate_from_file(self) -> bool:
        ...
        if license_data.get('expires_at'):
             if datetime.strptime(license_data.get('expires_at'), '%Y-%m-%dT%H:%M:%S').date() < date.today():
